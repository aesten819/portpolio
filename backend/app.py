...
def get_portfolio():
    """portfolio.json 파일에서 포트폴리오 데이터를 읽어 반환합니다."""
    ...
    try:
        ...
        return jsonify({"error": f"Failed to read portfolio file: {str(e)}"}), 500

@app.route('/api/portfolio', methods=['POST'])
def save_portfolio():
    """클라이언트로부터 받은 포트폴리오 데이터를 portfolio.json 파일에 저장합니다."""
    try:
...
def search_stock(query):
    """입력된 쿼리로 시작하는 종목명을 검색하여 상위 10개를 반환합니다."""
    if not query:
        return jsonify([])
